    ...
    return (
        <div className="w-full flex flex-col items-center pt-10 min-h-screen">
            ...
            <div className="relative flex items-center justify-between md:w-[50%] mb-8">
                <div className="absolute left-0 h-1 -z-10 top-[25%] w-[80%] md:w-[90%] bg-gray-300" />
                {[1, 2, 3].map((step, index) => (
                    <div key={step}>
                        <div className={`w-10 h-10 flex items-center justify-center rounded-full text-white font-bold ${step <= activeStep ? "bg-blue-600" : "bg-gray-300"}`}>{step}</div>
            ...
            <div className='md:w-[480px] p-8 bg-white shadow rounded-lg'>
                {activeStep === 1 && (
                    <>{!showOtp ? (<form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <input type='tel' placeholder='*********' className='w-full p-2 border border-gray-300 outline-0 mb-1 rounded-[4px]'
                        ...
                        />
                        {errors.phone_number && <p className='text-red-500 text-sm'>
                            {String(errors.phone_number.message)}
                        </p>}

                        <label className='block text-gray-700 mb-1'> Country</label>
                        <select className='w-full p-2 border border-gray-300 outline-0 mb-1 rounded-[4px]'
                            {...register("country", {
                        ...
                        <button type='submit' disabled={signUpMutation.isPending} className='w-full mt-4 text-lg cursor-pointer bg-black text-white py-2 rounded-lg'>
                            {signUpMutation.isPending ? "Signing up..." : "Signup"}
                        </button>

                        {signUpMutation.isError &&
                            signUpMutation.error instanceof AxiosError && (
                                <p className='text-red-500 text-sm'>{signUpMutation.error.message}</p>
                            )
                        }
                        ...
                        <div>
                            ...
                            <p className='text-center text-sm mt-4'>
                                {canResend ? (<button onClick={resendOtp} className='text-blue-500 cursor-pointer'>Resend otp</button>) : (`Resend Otp in ${timer} seconds`)}
                            </p>
                            {verifyOtpMutation.isError && verifyOtpMutation.error instanceof AxiosError && (
                                <p className='text-red-500 text-sm'>{verifyOtpMutation.error.message || verifyOtpMutation.error.response?.data?.message}</p>
                            )}
